<?php @session_start(); @ob_start(); ini_set('max_execution_time', 0); set_time_limit(0); @header("Content-Type: text/html; charset=utf-8"); @ob_end_clean();
error_reporting(1);
$starsArr = array();
$izin = "var";
...
$timetablepage = bot_curl("https://atilimengr.edupage.org/timetable/view.php");
...
if(count($test) < 1) die("Ders programı çekilemedi..");
//var_dump($test);
...
//die();
echo count($test["changes"][9]["rows"])."<hr>";
foreach($test["changes"] AS $key => $ss){
	echo $key." > ".$ss["table"]."<br>";
}


#PROGRAM NAME GÜZ BAHAR YIL
$shortname = explode(" ",$test["name"]);
$shortname = $shortname[0].' '.$shortname[1];
$db->query("INSERT INTO programs (name,short,updatedtime,verify) VALUES('".mysqli_real_escape_string($db,$test["name"])."','".mysqli_real_escape_string($db,$shortname)."',".time().",'".mysqli_real_escape_string($db,$test["ce_ttnum_last"])."')");


 #SUBJECTS
foreach($test["changes"][9]["rows"] AS $subjects){
...
#LESSONS
foreach($test["changes"][11]["rows"] AS $lesson){
	//var_dump($lesson);
...
#PERIODS
foreach($test["changes"][1]["rows"] AS $period){
	var_dump($period);
...
#DAYSDEF
foreach($test["changes"][3]["rows"] AS $daysdef){
...
#CLASSROOMS
foreach($test["changes"][6]["rows"] AS $classroom){
...
#CARDS
foreach($test["changes"][12]["rows"] AS $card){
...
#TEACHERS
foreach($test["changes"][7]["rows"] AS $teacher){
