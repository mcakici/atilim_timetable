...
function get_hour($value) {
    switch ($value) {
    case 1:$out = "09:30 - 10:20";
        break;
    case 2:$out = "10:30 - 11:20";
        break;
    case 3:$out = "11:30 - 12:20";
        break;
    case 4:$out = "12:30 - 13:20";
        break;
    case 5:$out = "13:30 - 14:20";
        break;
    case 6:$out = "14:30 - 15:20";
        break;
    case 7:$out = "15:30 - 16:20";
        break;
    case 8:$out = "16:30 - 17:20";
        break;
    case 9:$out = "17:30 - 18:20";
        break;
    }
    return $out;
}
...
function get_eng_shortdayname($value) {
    switch ($value) {
    case "Pazartesi":$out = "Mon";
        break;
    case "Salı":$out = "Tue";
        break;
    case "Çarşamba":$out = "Wed";
        break;
    case "Perşembe":$out = "Thu";
        break;
    case "Cuma":$out = "Fri";
        break;
    case "Cumartesi":$out = "Sat";
        break;
    case "Pazar":$out = "Sun";
        break;
    }
...
function get_days() {
    global $db;
    $query = $db->query("SELECT days.* FROM days WHERE id < 7");
    $result = array();
    while ($row_query = $query->fetch_object()) {
        $result[] = array(
            "id" => $row_query->id,
...
function get_classroom($classroomids = null) {
    global $db;
    if ($classroomids === null || (is_array($classroomids) && count($classroomids) == 0)) {
        return;
    } else if (!$classroomids || !is_array($classroomids) || empty($classroomids)) {
        $query = $db->query("SELECT classrooms.* FROM classrooms");
    } else if (is_array($classroomids) && $classroomids !== NULL) {
...
function get_subjects($type = null, $value = null) {
    ...
    if (isset($_COOKIE["_department"])) {
        ...
        if ($classinfo && count($classinfo) > 0) {
            ...
            $finalids = array_map(function ($d) {
                if ($d["id"] > 0) {
                    return "JSON_CONTAINS(lessons.classids, '" . intval($d["id"]) . "')";
                }
            }, $classinfo);
...
function get_lessoncount($selected_departments = null) {
    ...
    if ($selected_departments !== null && is_array($selected_departments)) {
        $finalClassids = array_map(function ($d) {
            if (intval($d) > 0) {
                return "JSON_CONTAINS(usedperiods.classids, '" . intval($d) . "')";
            }
        }, $selected_departments);
        $_query_extra = '
						WHERE usedperiods.dayid < 8  AND  (' . implode(" OR ", $finalClassids) . ')
						GROUP BY usedperiods.dp
						';
    } else {
        $_extra_query = " WHERE usedperiods.dayid < 8 GROUP BY usedperiods.dp ";
    }
...
function get_lessons($type = null, $value = null) {
    ...
    while ($row_sub = $query_subject->fetch_object()) {
        ...
        //var_dump($row_sub->classroomids);

        if ($type == "searchlesson") {
            $durationadded = $row_sub->duration - 1;
        } else {
            $durationadded = 0;
        }
...
function get_bologna($code) {
    ...
    if ($get_bologna == null) {
        ...
        if (!empty($get_bologna)) {
            $db->query("UPDATE subjects SET bologna='" . mysqli_real_escape_string($db, $get_bologna) . "' WHERE code='" . str_value($code) . "' ");
        }
...
function bot_curl($url) {
    ...
    curl_setopt($ch, CURLOPT_COOKIEJAR, dirname(__FILE__) . "/cache/cache.txt");
    curl_setopt($ch, CURLOPT_COOKIEFILE, dirname(__FILE__) . "/cache/cache.txt");
    curl_setopt($ch, CURLOPT_CAINFO, dirname(__FILE__) . "/cache/cacert.pem");
    $data = curl_exec($ch);
    return $data;
}
/*
POST /timetable/server/ttviewer.js?__func=getTTViewerData HTTP/1.1
Host: atilimengr.edupage.org
Connection: keep-alive
Content-Length: 41
User-Agent: Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/83.0.4103.116 Safari/537.36
DNT: 1
Content-type: application/json; charset=UTF-8
Origin: https://atilimengr.edupage.org
Sec-Fetch-Site: same-origin
Sec-Fetch-Mode: cors
Sec-Fetch-Dest: empty
Referer: https://atilimengr.edupage.org/timetable/?&lang=tr
Accept-Encoding: gzip, deflate, br
Accept-Language: tr-TR,tr;q=0.9,en-US;q=0.8,en;q=0.7
Cookie: PHPSESSID=256592d5a7fa6200ff86002de7aa4b12

 */
function bot_postcurl($url, $postfield = null) {
    $ch = curl_init();
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
    curl_setopt($ch, CURLOPT_FOLLOWLOCATION, TRUE);
    curl_setopt($ch, CURLOPT_MAXREDIRS, 2);
    curl_setopt($ch, CURLOPT_URL, $url);
    //if($justheader)    curl_setopt($ch, CURLOPT_HEADER, true);
    ...
    curl_setopt($ch, CURLOPT_REFERER, 'https://atilimengr.edupage.org/timetable/?&lang=tr');
    curl_setopt($ch, CURLOPT_HTTPHEADER, array("Origin: https://atilimengr.edupage.org", "Content-type: application/json; charset=UTF-8", "DNT: 1"
        , "Referer: https://atilimengr.edupage.org/timetable/?&lang=tr"));
    curl_setopt($ch, CURLOPT_COOKIEJAR, dirname(__FILE__) . "/cache/cache.txt");
    curl_setopt($ch, CURLOPT_COOKIEFILE, dirname(__FILE__) . "/cache/cache.txt");
    curl_setopt($ch, CURLOPT_CAINFO, dirname(__FILE__) . "/cache/cacert.pem");

    //if($current_proxy) curl_setopt($ch, CURLOPT_PROXY, $current_proxy);
    //curl_setopt($ch, CURLOPT_HTTPHEADER, array("X-Requested-With: XMLHttpRequest","Origin: https://www.dizibox.pw/","Host: dizibox.pw"));
